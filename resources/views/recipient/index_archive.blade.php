            ...
            <table id="recipientsTable" class="table table-striped nowrap" style="width: 100%">
                ...
                <tbody>
                    ...
                    <tr class="text-center">
                        ...
                        <td>{{ $archive->contact_information }}</td>
                        <td>{{ $archive->kin_fullname }}</td>
                        <td>{{ $archive->blood_type }}</td>
                        <td>{{ $archive->gender }}</td>
                        <td>{{ $archive->organ_needed }}</td>
                        <td>{{ $archive->status }}</td>

                        <td class="py-2 px-4 border-b flex gap-4 justify-center">
                            <a href="#" class="actionBtn show-recipient" data-id="{{ $archive->id }}" title="Show Recipient">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            @if(auth()->user()->role === 'SuperAdmin' || auth()->user()->role === 'Encoder')
                            <a href="{{ route('recipients.edit', $archive->id) }}" class="actionBtn" title="Edit Recipient">
